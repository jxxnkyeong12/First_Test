...
private static	SqlSession sql; //전송 결과 처리를 하는 객체 (sqlSessionFactory를 이용해서 초기화)
	static {  //제일먼저 실행되게 static걸어주기
		...
		try {
			...
			//sql= 열린 세션을 받아온다! ■이부분 인터넷에서 가져온거야 ㅎㅎ■
  ...
  public void test() {// sql.selectOne("mapperNamesapce.sql(id)")
	...
	public List<CustomerDTO> getList() { //List가 ArrayList의 부모클래스 라서 기능이 다 똑같아
		// sql.update, sql.delete, sql.selecton (한건만 조회할때), selectList (리턴타입이 List 니까 그냥 list로 써주면 받아주면 돼~)
