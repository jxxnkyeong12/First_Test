...
오라클 조인
SELECT e.employee_id, e.first_name || ' ' || e.last_name name, d.department_name
FROM employees e, departments d
WHERE e.department_id = d.department_id
...
FROM departments d INNER JOIN locations l
USING (location_id)
WHERE location_id = 1400; --밑에껄 l.location_id가 아니라 l지워주니 된다
...
오라클 조인
SELECT e.employee_id, e.last_name, e.manager_id, m.last_name, m.hire_date
FROM employees e, employees m
WHERE e.manager_id = m.employee_id
