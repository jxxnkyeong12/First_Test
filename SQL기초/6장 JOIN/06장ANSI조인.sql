...
ANISI JOIN ON
SELECT e.employee_id, e.first_name, e.department_id, d.department_name
FROM employees e INNER JOIN departments d  --null값 빼고 나와
ON e.department_id = d.department_id;   --106개 나와

ANISI JOIN USING
SELECT e.employee_id, e.first_name, department_id, d.department_name   --Using은 'e'.department_id 앞에 테이블명이 들어가면 안돼, 그래서 없애줘!
...
--USING (department_id)
WHERE e.department_id = 80;   --34개 추가는 where로 해줘!

2)ANSI JOIN USING --얘는 식별자를 앞에 안두네....? e.이나 d.같은... 응 안둬! 
